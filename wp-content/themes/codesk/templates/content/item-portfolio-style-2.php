<?php 
   ...
   if(!empty($item_cats) && !is_wp_error($item_cats)){
      foreach((array)$item_cats as $item_cat){
         ...
         $output .= '<a href="'.get_category_link( $item_cat->term_id ).'" title="' . esc_attr( sprintf( esc_attr__( "View all posts in %s", 'codesk' ), $item_cat->name ) ) . '">'.$item_cat->name.'</a>'.$separator;
      }
...
<div class="<?php echo esc_attr($item_classes) ?>">
   <div class="portfolio-block portfolio-v2">      
      ...
      <div class="portfolio-content">
         ...
         <div class="readmore">
            <a class="btn-inline" href="<?php the_permalink(); ?>"><?php echo esc_html__( 'Read More', 'codesk' ) ?></a>
         </div>
      </div>   
   </div>
</div>
